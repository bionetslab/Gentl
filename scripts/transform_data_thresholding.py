...
def draw_and_store_bounding_rectangles_within_images():
    """
    Draw rectangular boxes (outer and inner) within image contours to extract features from
    A CSV file containing patient IDs and the coordinates of the bounding boxes
    """
    ...
    for root, _, files in os.walk(dataset_path):
        for file in files:
            if file.endswith('.jpg'):  # Check if the file is a .jpg file
                ...
                # Step 4: Get the largest contour
                largest_contour = max(contours, key=cv2.contourArea)

                # Step 5: Calculate bounding rectangle
                ...
                output_file_path = os.path.join(output_path, file)  # Save with the same name
