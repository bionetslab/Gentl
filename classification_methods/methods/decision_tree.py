...
def classify_cancer_invasion(selected_feature, max_no_of_rois, gentl_result_param, gentl_flag):
    ...
    best_parameters = load_best_params(
        task, selected_feature, max_no_of_rois, gentl_result_param, gentl_flag, "decision_tree_best_params_new.csv"
        )
    # Create Decision Tree classifier
    model = Pipeline(
        [
            ('scaler', StandardScaler()),  # Optional: Decision trees don't need scaling, but kept for consistency
...
def hyperparameter_tuning(task, X, y, max_no_of_rois, selected_feature, gentl_flag, gentl_result_param=None):
    ...
    append_hyperparams_to_csv(
        "decision_tree", task, selected_feature, max_no_of_rois, gentl_result_param, gentl_flag, best_params,
        "decision_tree_best_params_new.csv"
        )
    # # Save best parameters and performance to a text file
    # with open("dt_best_model.txt", "a") as file:
