...
def evaluate_model_cv(model, X, y, model_name, feature_column, cv=LeaveOneOut()):
    """
    Evaluate a model using Leave-One-Out Cross Validation.
    Prints the average weighted F1 score and its standard deviation,
    and returns a tuple (mean, std) (both in percentage).

    :param model: The model to evaluate.
    :param X: Feature matrix.
    :param y: Labels.
    :param model_name: Name of the model.
    :param feature_column: The feature column used in the model.
    :param cv: Cross-validation strategy (default is LeaveOneOut()).
    :return: Tuple (mean F1 score, std F1 score) in percentage.
    """
...
# Each classifier function uses leave-one-out CV and returns (mean, std).
...
# Run all classifiers using leave-one-out CV and return a dictionary mapping ML method -> (mean, std)
...
def process_file(file_path, metric_type, glcm_feature, cancer_grades_df):
    ...
    :return: Tuple (merged_df, feature_column) where feature_column is used for classification.
    """
...
def plot_results(results, metric_type):
    ...
    # Place legend at the top outside the plot; adjust bbox if needed.
    ...
    pdf_filename = f"barplot_leave_one_out_cv_{metric_type}.pdf"
...
if __name__ == "__main__":
    ...
    for metric in METRIC_TYPES:
        for feature in GLCM_FEATURES:
            ...
            # Run all classifiers using leave-one-out CV and get (mean, std)
